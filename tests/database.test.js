...
test('openDatabase creates a healthy SQLite database', async () => {
  ...
  try {
    ...
    assert.equal(database.get('SELECT COUNT(*) AS count FROM schema_migrations').count, 4);
    database.close();

    const reopenedDatabase = await openDatabase(databasePath);
    assert.equal(reopenedDatabase.get('SELECT COUNT(*) AS count FROM schema_migrations').count, 4);
    assert.ok(
