...
router.get('/:id', async (req, res) => {
    ...
    try {
        const [orderResult, itemsResult] = await Promise.all([
            ...
            db.query(
                `SELECT id, listing_id, quantity, unit_price
...
router.post('/', async (req, res) => {
    ...
    const shippingAddress = typeof req.body.shipping_address === 'string'
        ? req.body.shipping_address.trim()
        : '';

    if (!userId || !items || !shippingAddress) {
        return res.status(400).json({
            message: 'user_id, shipping_address, and at least one valid item are required.',
        });
    }
    ...
    try {
        ...
        const orderResult = await client.query(
            `INSERT INTO orders (user_id, status, total_amount, shipping_address, payment_status)
             VALUES ($1, 'pending', $2, $3, 'pending')
             RETURNING id, user_id, status, total_amount, shipping_address, payment_status, created_at`,
        ...
        for (const item of items) {
            const itemResult = await client.query(
                `INSERT INTO order_items (order_id, listing_id, quantity, unit_price)
                 VALUES ($1, $2, $3, $4)
                 RETURNING id, listing_id, quantity, unit_price`,
                [order.id, item.listingId, item.quantity, item.unitPrice],
            );
    ...
    try {
        const result = await db.query(
            `UPDATE orders
             ...
             WHERE id = $2
             RETURNING id, user_id, status, total_amount, shipping_address,
                       payment_status, created_at, updated_at`,
            [status, orderId],
        );
